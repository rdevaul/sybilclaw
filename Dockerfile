...
COPY --from=runtime-assets --chown=node:node /app/sybilclaw.mjs .
...
RUN ln -sf /app/sybilclaw.mjs /usr/local/bin/openclaw \
 && chmod 755 /app/sybilclaw.mjs
...
# Create required directories for the node user before switching to non-root
RUN mkdir -p /home/node/.sybilclaw/devices && chown -R node:node /home/node/.sybilclaw
...
CMD ["node", "sybilclaw.mjs", "gateway", "--allow-unconfigured"]
