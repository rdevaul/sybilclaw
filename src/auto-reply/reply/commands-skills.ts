 ...
 *
 * Re-expressed 2026-08-21 against v2026.6.34: skill helpers moved from
 * src/agents/skills.ts -> src/skills/loading/workspace.ts and the flat
 * `filterWorkspaceSkillEntries(entries, cfg)` became
 * `filterWorkspaceSkillEntriesWithOptions(entries, { config })`;
 * `resolveEffectiveAgentSkillFilter` moved to src/skills/discovery/agent-filter.ts.
 */
...
export const handleSkillsCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  try {
    ...
    const allNames = loadAllEntries(params.workspaceDir, params.cfg).map(skillEntryName).toSorted();
