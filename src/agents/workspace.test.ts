...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENCLAW_HOME for default workspace resolution", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/openclaw-home"), ".sybilclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".sybilclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".sybilclaw"), { recursive: true });
    await fs.writeFile(
